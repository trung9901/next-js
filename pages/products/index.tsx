        ...
        {products.map((item: any) => (
          <div key={item.id} className="flex gap-3">
            ...
            <div className="bg-red-500 p-2 rounded-md text-white ">
              <button onClick={() => { remove(item.id), console.log(item.id) }}>Delete</button>
            </div>
            <div className="bg-cyan-500 p-2 rounded-md text-white ">
              <button onClick={() => { update(item.id, { id: 99, name: "Product update2" }) }}>Update</button>
            </div>
          </div>
        ))}
        <div >
          <button className="bg-green-500 p-2 rounded-md text-white" onClick={() => create({ id: 99, name: "Product new" })}>Create</button>
        </div>
      </div>
    </div>
  );
